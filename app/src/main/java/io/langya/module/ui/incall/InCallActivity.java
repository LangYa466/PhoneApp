package io.langya.module.ui.incall;
...
import io.langya.module.call.CallController;
import io.langya.module.call.CallControllerListener;
import io.langya.module.callerid.CallerIdCache;
import io.langya.module.callerid.CallerIdLookup;
...
public class InCallActivity extends AppCompatActivity implements CallControllerListener {
    ...
    /** 当前正在显示的号码 —— 用作异步回调的有效性判据 避免旧请求覆盖新号码 */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        b.btnHangup.setIcon(Md3Icons.of(this, "mso-call_end"));
        b.btnReject.setIcon(Md3Icons.of(this, "mso-call_end"));
        b.btnAnswer.setIcon(Md3Icons.of(this, "mso-call"));
        b.btnHangup.setOnClickListener(v -> CallController.get().hangup(primary()));
        b.btnAnswer.setOnClickListener(v -> CallController.get().answer(primary()));
        b.btnReject.setOnClickListener(v -> CallController.get().reject(primary()));
    ...
    protected void onStart() {
        super.onStart();
        CallController.get().addListener(this);
    ...
    protected void onStop() {
        super.onStop();
        CallController.get().removeListener(this);
    ...
    private Call primary() { return CallController.get().primary(); }
    ...
    private void applyEdgeInsets() {
        ViewCompat.setOnApplyWindowInsetsListener(b.getRoot(), (v, insets) -> {
            ...
            ((ViewGroup.MarginLayoutParams) b.hangupRow.getLayoutParams())
    ...
    private void bindDock(InCallDockItemBinding dock, String iconKey, int labelRes, View.OnClickListener l) {
        dock.dockIcon.setIcon(Md3Icons.of(this, iconKey));
    ...
    private void syncDockSelection() {
        var s = CallController.get().getAudioState();
        ...
        b.dockMute.dockIcon.setIcon(Md3Icons.of(this,
    ...
    private void toggleMute() {
        var s = CallController.get().getAudioState();
        CallController.get().setMuted(s == null || !s.isMuted());
    }

    private void toggleSpeaker() {
        var s = CallController.get().getAudioState();
        boolean on = s != null && (s.getRoute() & CallAudioState.ROUTE_SPEAKER) != 0;
        CallController.get().setSpeaker(!on);
    }
    ...
    private void showDialpadSheet() {
        ...
        MaterialButton btnClose = view.findViewById(R.id.btnCloseDialpad);
    ...
    private void wireDtmf(View parent, int rootId, char digit, String sub) {
        ...
        item.setOnClickListener(v -> {
            CallController.get().sendDtmf(primary(), digit);
            if (dtmfDisplay != null) {
    ...
    private void showMoreSheet() {
        ...
        MaterialButton btnClose = view.findViewById(R.id.btnCloseMore);
        btnClose.setIcon(Md3Icons.of(this, "mso-close"));
    ...
    private void render() {
        ...
        if (safeNumber.isEmpty()) {
            ...
            var cached = CallerIdCache.get(safeNumber);
            if (cached != null && !cached.isEmpty()) {
                b.tvCallerId.setText(cached);
            } else if (cached != null) {
                b.tvCallerId.setText("");
            } else {
                new CallerIdLookup().query(this, safeNumber, result -> runOnUiThread(() -> {
                    if (isFinishing() || isDestroyed()) return;
